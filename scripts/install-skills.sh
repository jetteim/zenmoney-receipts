...
if [[ "${1:-}" == "--refresh" ]]; then
  ...
  print -u2 "Usage: ./scripts/install-skills.sh [--refresh]"
  exit 2
fi
...
for skill_name in categorize-zenmoney-receipts review-zenmoney-categories find-zenmoney-savings; do
  ...
  if [[ ! -d "$target" ]]; then
    ...
    if ! grep -q "^name: $skill_name$" "$target/SKILL.md"; then
      print -u2 "Refusing to refresh unrecognized skill directory: $target"
      exit 2
    fi
...
if (( ${#refresh_names[@]} > 0 )); then
  print "Refreshed ${#refresh_names[@]} ZenMoney agent skills from this clone."
fi
