...
ZenMoney tags have no archive field. Retirement sets `showIncome`, `showOutcome`, `budgetIncome`, and `budgetOutcome` to `false`; it neither deletes the tag nor changes existing transaction tags. Category creation and updates enforce ZenMoney's maximum one parent level and fail closed if the complete taxonomy exceeds the 500-record safety bound. Bulk category consolidation/history migration is not exposed in version 0.4.0.
