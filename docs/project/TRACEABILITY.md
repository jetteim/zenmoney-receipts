...
| VS-05 / C-17 receipt-informed taxonomy memory | F-017 opt-in evidence store | Atomic minimal local state; verified-write evidence binding; narrow purpose validation; bounded inspect/delete/purge; three-receipt readiness | `tests/receipt-memory.test.ts`, `tests/receipt-operations-service.test.ts`, MCP contract, CLI smoke, `docs/evidence/2026-08-17-v0.6.0-receipt-memory.md` | Complete offline; local enablement verified |
